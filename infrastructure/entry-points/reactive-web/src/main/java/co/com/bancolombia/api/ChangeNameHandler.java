...
import co.com.bancolombia.usecase.user.ExecuteChangeNameUseCase;
...
public class ChangeNameHandler {
    ...
    private final ExecuteChangeNameUseCase useCase;
    ...
    private Mono<ServerResponse> handleError(Throwable error) {
        ...
        if (error instanceof SqlExecutionInfrastructureException) {
            log.error("Infrastructure error during change name operation", error);
            return ServerResponse.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .bodyValue(new ErrorResponse("Internal server error"));
        }
        log.error("Unexpected error during change name operation", error);
        return ServerResponse.status(HttpStatus.INTERNAL_SERVER_ERROR)
                .bodyValue(new ErrorResponse("Internal server error"));
    }
}
